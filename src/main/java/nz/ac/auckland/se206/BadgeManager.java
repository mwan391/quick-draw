...
public class BadgeManager {
  ...
  public static void initializeBadges() {
    ...
    availBadges.add(badge);
    badge = new BadgeModel(6, "Lightning!", "You finished a game under 5 seconds! You're cracked!");
    availBadges.add(badge);
  ...
  public static boolean checkNewBadges(String username, List<BadgeModel> userBadges, boolean won) {
    boolean hasNewBadge = false;
    UserDaoJson userDao = new UserDaoJson();

    // check for first game
    if (!userBadges.contains(availBadges.get(0))) {
      userDao.addBadge(availBadges.get(0), username);
      hasNewBadge = true;
    }

    // check for first win badge
    if (won && !userBadges.contains(availBadges.get(1))) {
      userDao.addBadge(availBadges.get(1), username);
      hasNewBadge = true;
    }

    // check for first lose badge
    if (!won && !userBadges.contains(availBadges.get(2))) {
